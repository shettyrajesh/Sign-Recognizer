...
class SelectorDIC(ModelSelector):
    ...
    def select(self):
        ...
        selected_model = None
        selected_model_dic_value = float("-inf")
        current_dic_value = 0
        sum_log_p = 0
        logP_list = {}
        ...
        try:
            ...
            M = len(logP_list)
            for n in logP_list:
                current_dic_value =  logP_list[n] - 1/(M - 1)*(sum_log_p-logP_list[n])
                if current_dic_value > selected_model_dic_value:  # higher scores are better
                        selected_model = self.base_model(n)
        ...
        for n in range(self.min_n_components, self.max_n_components + 1):
            try:
                if len(self.sequences) > 2:
                    ...
                    hmm_model = self.base_model(n)
                    current_cv_value = hmm_model.score(self.X, self.lengths)
            except:
                pass
